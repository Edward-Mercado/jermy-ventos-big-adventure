<template>
    <div
        class="top-0 left-0 absolute w-full p-2 h-full bg-linear-to-tr from-sky-100/96 to-white/96 rounded-2xl flex flex-col justify-between z-10">
        <button
        ...
        <div class="h-[90%] flex flex-col gap-4">
            ...
            <p class="text-2xl text-slate-700 pixfont italic">LV. {{ campaignStore.playerLevel }} Birthday {{ getTitle()
                }}</p>
            <p class="text-md text-slate-700 pixfont italic">It's the man's birthday and he's got something to prove.
            </p>
            ...
            <battles-health-bar :currentHP="campaignStore.currentMana" :maxHP="campaignStore.maxMana" :isMana="true"
                :nextLevel="campaignStore.playerLevel + 1"></battles-health-bar>
            <p class="text-2xl text-black pixfont">EXP:</p>
            <battles-health-bar :currentHP="campaignStore.expGained"
                :maxHP="campaignStore.playerLevelData[campaignStore.playerLevel + 1]!.expRequirement" :isEXP="true"
                :nextLevel="campaignStore.playerLevel + 1"></battles-health-bar>
        </div>
    </div>
</template>
