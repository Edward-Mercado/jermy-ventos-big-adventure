<template>
    <div class="w-[35%] flex flex-col justify-between relative overflow-hidden"> <!-- JOEY CONTAINER-->
        ...
        <div class="w-[35%] absolute bottom-2 right-2 bg-slate-600/70 rounded-lg h-[65%] border-2 border-black shadow-md shadow-slate-900/20 p-2 flex flex-col gap-2">
            ...
            @click="clickSFX(); viewAbilities=true">Abilities</button>
            <div class="bg-slate-900 h-[50%] w-full rounded-md mt-2 shadow-lg flex flex-wrap">

            </div>
            <button class="w-full text-black bg-white rounded-md pixfont text-[0.92rem]
            transition-all duration-300 ease-in-out hover:bg-slate-100 active:bg-slate-300 shadow-md hover:shadow-lg active:shadow-none
            hover:-translate-y-0.5 active:translate-y-px"
            @click="clickSFX()">Confirm</button>
        </div>
        <transition name="modal">
            <battles-joey-checker v-if="checking" @close="checking = false"></battles-joey-checker>
        </transition>
        <transition name="modal">
            <battles-abilities-modal v-if="viewAbilities" @close="viewAbilities = false"></battles-abilities-modal>
        </transition>
    </div>
</template>
