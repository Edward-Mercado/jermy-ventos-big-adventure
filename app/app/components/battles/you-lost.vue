<template>
    <div class="w-full h-dvh flex flex-col items-center">
        <h2 class="shaky text-black pixfont text-[5rem] text-center w-full uppercase">You lost...</h2>
        <div class="w-[60%] flex flex-col items-center bg-slate-800/70 h-[70%] gap-4 border-4 border-black p-2 relative">
            <h4 class="pixfont text-white text-[3rem]">Enemies Fought:</h4>
            <p class="text-sky-200 dialogue-font text-2xl" v-for="enemy in battleData!.enemies">{{ enemy.name }} </p>
            ...
            @click="()=>{clickSFX(); $emit('retry')}">Retry</button>
        </div>
    </div>
</template>
